...
`/management/` にアクセスすると、未ログインの場合は専用のログイン画面
(`/accounts/login/`) に移動します。有効なスタッフアカウントでログインしてください。
管理画面のヘッダーにはユーザー名、パスワード変更、ログアウトを表示します。
パスワード変更には現在のパスワードが必要で、変更後もログイン状態を維持します。

パスワードを忘れた場合はログイン画面の「パスワードをお忘れですか？」から、
管理用アカウントに登録されているメールアドレスを入力してください。
会員名簿のメールアドレスとは別に、Django管理サイトのユーザー設定で登録が必要です。
メールのリンクから新しいパスワードを設定できます。リンクはDjango標準設定で3日間有効で、
使用後は無効になります。登録の有無にかかわらず同じ送信完了案内を表示します。

本番の再設定メールは既存の `ELECTION_EMAIL_*` と `ELECTION_FROM_EMAIL` の設定を使います。
開発環境ではメールは実送信せず、本文と再設定リンクをサーバーのコンソールへ出力します。
...
* 選挙権: 正会員
* 一般枠と企業枠を別々の選挙として実施
* 一般枠の候補者は、企業所属を含む正会員全員
* 企業枠の候補者は、企業所属の正会員のみ（一般枠にも重複して登録）
...
それ以外は名簿上の所属枠を一般枠として扱います。
代議員の一般枠予備選挙では、この所属枠にかかわらず正会員全員を候補者とします。
企業所属の会員も、一般枠の予備選挙で3票以上得票すれば一般枠の本選挙へ進出します。
...
会員名簿の取り込み後、管理画面で予備選挙を新規追加すると、
その年度の選挙権を持つ会員が候補者として自動生成されます。
本選挙の追加時や、既存の予備選挙の編集時には生成されません。

以下のコマンドは、候補者を手動で再生成・確認する場合に利用できます。
...
既存の一般枠予備選挙にも上記コマンドで不足する候補者を追加できます。
登録済み候補者やそのステータスは保持され、重複登録されません。
...
管理画面で選挙を新規追加すると、その年度の選挙権を持つ会員が
有権者として自動生成されます。予備選挙・本選挙の両方が対象です。
既存の選挙を編集した場合は再生成されません。

以下のコマンドは、有権者を手動で再生成・確認する場合に利用できます。
...
`開票プレビュー` を押すと、DBを変更せずに開票結果を計算します。
開票済みの選挙でもプレビューを閲覧できます。
...
最多得票者が同票の場合は、開票確定時に候補者を抽選対象として登録します。
`LotteryDraw` の管理画面から抽選を実行すると、1名が当選となります。

---

# 41. 最終結果 CUI

開票済みの予備選挙・本選挙は、年度別選挙管理ページの「選挙結果」、または
Django Adminの選挙詳細画面にある「結果CSVをダウンロード」からCSVを保存できます。
予備選挙のCSVには得票数と本選挙進出基準の判定（本選挙進出／基準未達）を出力し、
ファイル名に `preliminary` を付けて本選挙と区別します。未実行の抽選がある場合は、
...
候補者管理ページで「資格なし」を選んで更新すると、
その選挙の候補者レコードを削除します。本選挙の「候補者を削除」ボタンも同じ動作です。
会員名簿と変更履歴は残ります。削除できるのは準備中の選挙のみで、
投票・抽選データが参照する候補者や開票確定済みの候補者は削除できません。
予備選挙の候補者生成を再実行すると、削除した会員も資格条件に合えば再登録されます。

予備選挙で選択できる状態は「被選挙人」「資格なし」のみです。本選挙では「辞退」も選択できます。

## 開発時のコード構成

責務の分担と検証コマンドは [コード構成と変更時の確認](docs/architecture.md) を参照してください。

## ライセンス

本プロジェクトは [MIT License](LICENSE) のもとで公開しています。

Copyright (c) 2026 日本加速器学会
